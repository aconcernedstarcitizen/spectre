...
4. **Add your sale times** - Copy the times from the CIG website and paste them.
   ...
   **Standard CIG wave times** (for January 15, 2025 as an example):
   ```yaml
   sale_windows:
     - "2025-01-15 16:00"  # Wave 1: 4:00 PM UTC
     - "2025-01-15 20:00"  # Wave 2: 8:00 PM UTC
     - "2025-01-16 00:00"  # Wave 3: 12:00 AM UTC (next day)
     - "2025-01-16 04:00"  # Wave 4: 4:00 AM UTC (next day)
     - "2025-01-16 08:00"  # Wave 5: 8:00 AM UTC (next day)
     - "2025-01-16 12:00"  # Wave 6: 12:00 PM UTC (next day)
   ```
...
**When you're ready for the real sale:**
...
8. **If First Wave Fails** (automatically moves to next wave)
   ```
   ❌ Wave 1: Checkout unsuccessful
   ➡️  Moving to Wave 2...
   ...
   ⏳ Waiting 3h 45m 12s until pre-wave activation...
   ```
   - Process repeats for Wave 2, Wave 3, etc.
   - App runs continuously through all waves until success
...
2. **Pre-Wave Polling**: Starting 2 minutes before each wave, sends HTTP HEAD requests every second checking for 200 status (product available)
3. **SKU Extraction**: Once page is available, extracts SKU ID from page HTML via regex pattern matching
4. **API-Based Checkout**: Bypasses browser UI entirely, sends direct GraphQL mutations to RSI's store API
5. **Smart Retry**: Implements exponential backoff for rate limits, specific delays for different error types (4226, 4227, out of stock, etc.)
6. **Cart Validation**: Detects if cart already has correct item with credits applied, skips redundant steps
7. **Address Caching**: Pre-fetches and caches billing address to eliminate lookup delays during checkout
8. **reCAPTCHA v3**: Generates fresh Enterprise tokens for each cart addition attempt
9. **Multi-Wave State Machine**: Automatically transitions between waves on timeout, exits gracefully on success
...
4. **Добавьте времена продаж** - Скопируйте времена с сайта CIG и вставьте их.
   ...
   **Стандартные времена волн CIG** (для 15 января 2025 в качестве примера):
   ```yaml
   sale_windows:
     - "2025-01-15 16:00"  # Волна 1: 16:00 UTC (4:00 PM)
     - "2025-01-15 20:00"  # Волна 2: 20:00 UTC (8:00 PM)
     - "2025-01-16 00:00"  # Волна 3: 00:00 UTC (12:00 AM следующий день)
     - "2025-01-16 04:00"  # Волна 4: 04:00 UTC (4:00 AM следующий день)
     - "2025-01-16 08:00"  # Волна 5: 08:00 UTC (8:00 AM следующий день)
     - "2025-01-16 12:00"  # Волна 6: 12:00 UTC (12:00 PM следующий день)
   ```
...
**Когда вы готовы к настоящей продаже:**
...
8. **Если первая волна не удалась** (автоматически переходит к следующей волне)
   ```
   ❌ Волна 1: Оформление не удалось
   ➡️  Переход к волне 2...
   ...
   ⏳ Ожидание 3ч 45м 12с до активации перед волной...
   ```
   - Процесс повторяется для Волны 2, Волны 3 и т.д.
   - Приложение работает непрерывно через все волны до успеха
...
2. **Опрос перед волной**: Начиная за 2 минуты до каждой волны, отправляет HTTP HEAD запросы каждую секунду, проверяя статус 200 (продукт доступен)
3. **Извлечение SKU**: Как только страница доступна, извлекает SKU ID из HTML страницы через поиск по регулярному выражению
4. **Оформление заказа через API**: Полностью обходит UI браузера, отправляет прямые GraphQL мутации к API магазина RSI
5. **Умные повторные попытки**: Реализует экспоненциальную задержку для ограничений скорости, специфические задержки для различных типов ошибок (4226, 4227, нет на складе и т.д.)
6. **Проверка корзины**: Определяет, есть ли в корзине уже правильный товар с примененными кредитами, пропускает избыточные шаги
7. **Кэширование адреса**: Предварительно получает и кэширует адрес для выставления счета, чтобы устранить задержки поиска во время оформления заказа
8. **reCAPTCHA v3**: Генерирует свежие Enterprise токены для каждой попытки добавления в корзину
9. **Мультиволновая машина состояний**: Автоматически переходит между волнами по таймауту, корректно завершается при успехе
